...
export default function QuestsHeader(section: QuestsHeaderProps): React.ReactNode {
  let value = roundTo(section.totalPoints / section.totalWeight, 2);
  value = !Number.isNaN(value) ? value : +``;

  const { windowWidth }: { windowWidth: number } = WindowSize.useWidth();

  return (
    <div className="bg-opacity-70 relative h-11 bg-[#6B6C76] bg-blend-overlay select-none">

...
export default function Details(props: DetailsProps): React.ReactElement {
  ...
  const identifiers: React.ReactElement = (
  ...
  if (daily.requirements !== null) {
    let nextRequiredCompletionDate: Option<CalendarDate> = null;
    ...
    if (
      ...
      weeklyQuestStats?.isWeeklyRequirementComplete
    )
      nextRequiredCompletionDate = parseDate(weeklyQuestStats.latestCompleteDate!).add({
    ...
    description = (
      <PopoverText
        maxLines={2}
        popoverContentProps={{
          className: "border-1 border-white bg-black/60",
        }}
        popoverTextProps={{
        ...
        text={
          <span>
            {nextRequiredCompletionDate && (
              <>
                <span>Next req. date: {`${nextRequiredCompletionDate} `}</span>
                <br></br>
              </>
            )}
        ...
        textClassValue="my-[2] text-[0.60rem] leading-none tracking-tighter text-ellipsis"
      />
    );
  } else {
    description = (
      <PopoverText
        maxLines={2}
        popoverContentProps={{
          className: "rounded-none border-1 border-white bg-black/60",
        }}
        popoverTextProps={{
          className: "text-xs text-white",
        }}
        text={(descriptionContent == "note" ? daily.note : daily.description) ?? ""}
        textClassValue="mt-[3] text-[0.60rem] leading-none tracking-tighter text-ellipsis"
      />
    );
  }
...
type PopoverTextProps = {
  ...
  text: string | React.ReactElement<"span">;
